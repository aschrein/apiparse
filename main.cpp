...
#undef DISPATCH
  ...
  void print(Decl *D, raw_ostream &Out, const PrintingPolicy &Policy,
    ...
    DeclPrinter Printer(Out, Policy, D->getASTContext(), Indentation,
      PrintInstantiation);
...
#include "clang/Basic/AttrList.inc"
  ...
  void DeclPrinter::VisitFunctionDecl(FunctionDecl *D) {
    ...
    if (D->isPure()) {
      ...
      Out << " override { return _wrapped->" << D->getNameAsString() << "(";
      auto params = D->parameters();
      for (int i = 0; i < params.size(); i++) {
        Out << params[i]->getNameAsString() << (i == params.size() - 1 ? "" : ", ");
      }
      Out << ");}";
    }
  ...
  bool VisitDecl(clang::Decl *D)
  {
    ...
    if (auto const *pFD = llvm::dyn_cast<clang::FunctionDecl>(D))
    {
      ...
      if (print) {
        ...
        g_ss << "####################################################\n";
        g_ss << "FUNC_BEGIN(ctx, \"" << pFD->getNameAsString() << "\")\n";
        std::string stmt;
        llvm::raw_string_ostream stream(stmt);
        pFD->getReturnType().print(stream, PrintingPolicy(LangOptions()));
        stream.flush();
    ...
    if (auto *pRD = llvm::dyn_cast<clang::CXXRecordDecl>(D))
    {
      print = 0
      ...
      if (print)
      {
        ...
        g_ss << "#################### CLASS #########################\n";
        g_ss << "####################################################\n";
        g_ss << "CLASS_BEGIN(ctx, \"" << pRD->getNameAsString() << "\")\n";
        for (CXXRecordDecl::base_class_iterator Base = pRD->bases_begin(),
          BaseEnd = pRD->bases_end(); Base != BaseEnd; ++Base) {
          std::string stmt;
          llvm::raw_string_ostream stream(stmt);
          Base->getType().print(stream, PrintingPolicy(LangOptions()));
          stream.flush();
          g_ss << "INHERITS(ctx, \"" << stmt << "\")\n";
        }
        for (DeclContext::decl_iterator D = pRD->decls_begin(), DEnd = pRD->decls_end();
          D != DEnd; ++D) {
        ...
        g_ss << "CLASS_END(ctx, \"" << pRD->getNameAsString() << "\")\n";
      }
    }
    ...
    /*if (print)
    {
      ...
      WrapperPrinter::print(D, stream, PrintingPolicy(LangOptions()), 0, false);
...
int main(int argc, const char **argv) {
  ...
  if (!ret)
  {
    ...
    out << "from apiutil import *\n";
    out << "from d3d11wrapgen import *\n";
    out << "ctx = APIContext()\n";
    out << g_ss.str();
    out << "genWrappers(ctx)\n";
    out.close();
